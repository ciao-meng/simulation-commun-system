...
subplot(2, 1, 2); plot(W/(2*pi), F_10Hz); title('10Hz采样信号频域');

%% 从10Hz采样信号恢复
f_uncovery = f_10Hz;
B = sinc((1/Nsampling) * (ones(length(ts), 1) * t - ts' * ones(1, length(t))));%构造内插函数
                                                   %妙蛙妙蛙，值得学习这种构造求和过程的思想
f_covery = f_uncovery * B;%插值重构

figure;
subplot(2, 1, 1); plot(t, origin); title('原信号时域');
subplot(2, 1, 2); plot(t, f_covery); title('10Hz信号恢复');
