...
for k=1:length(delay)            
  [BER_MC(k) a]= c214_QPSKrun(N,Eb,...
       No,ChannelAttenuation,delay(k),0,0,0);
  disp(['Simulation ',...
       num2str(k*100/length(delay)),'% Complete']);
end
% BER_T = erfc(sqrt(EbNo))*ones(size(delay)); % Theoretical BER QDPSK
BER_T = 0.5*erfc(sqrt(EbNo))*ones(size(delay)); % Theoretical BER QPSK
figure
semilogy(delay,BER_MC,'o',delay, BER_T,'-')    %Plot BER vs Delay
xlabel('Delay (symbols)'); ylabel('Bit Error Rate'); 
axis([min(delay) max(delay) 0.01 1])
