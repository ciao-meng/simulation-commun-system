...
for k=1:length(SymJitter)
  BER_MC(k) = c214_QPSKrun(N(k),Eb,No,ChannelAttenuation,0,SymJitter(k),0,0);
  disp(['Simulation ',num2str(k*100/length(SymJitter)),'% Complete']);
end
figure
semilogy(SymJitter,BER_MC,'o',SymJitter,BER_T,'-')
axis([min(SymJitter) max(SymJitter) 0.01 0.08])
